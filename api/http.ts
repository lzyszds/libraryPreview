...
instance.interceptors.response.use((response: any): any => {
  ...
  if (response.data.code === 200) {
    ...
    return Promise.reject(identifyCode(response.data.code, response))
  }
})

// 将自定义的HttpResonse接口导出
export default async function http<T = any>(method = 'get', url = '', data = {}, headers: any = {}): Promise<HttpResonse<T>> {
  // 设置默认头部信息
